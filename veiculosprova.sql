...
#Selecione todos os veículos da tabela veículos.
select *
FROM veiculos;

#Selecione os nomes e e-mails de todos os clientes da tabela clientes.
select nome, email
FROM clientes;

#Selecione o modelo e a cor de todos os veículos que são azuis.
select modelo, cor
from veiculos
where cor = "azul";

#Selecione o nome do cliente e a data da venda para todas as vendas.
select clientes.nome 
as nome_cliente, vendas.data_venda 
from vendas 
INNER JOIN clientes ON vendas.id_cliente = clientes.id;
 
#Selecione os clientes que compraram veículos em 2023.
select clientes.*
from clientes
INNER JOIN vendas ON clientes.id = vendas.id_cliente
where year(vendas.data_venda) = 2023;
...
#Selecione o modelo, a marca e o ano dos veículos vendidos. 
select veiculos.modelo, marcas.nome 
as marca, veiculos.ano 
from vendas 
Inner join veiculos ON vendas.id_veiculo = veiculos.id
INNER join marcas ON veiculos.id_marca = marcas.id;

#Selecione o número total de veículos vendidos em 2023.
select count(*)
as total_veiculos_vendidos_2023 
from vendas where year(data_venda) = 2023;

#Selecione o cliente e o veículo vendido em cada venda. 
select clientes.nome as nome_cliente, veiculos.modelo as modeloveiculo
from vendas
INNER JOIN clientes ON vendas.id_cliente = clientes.id
INNER JOIN veiculos ON vendas.id_veiculo = veiculos.id;
